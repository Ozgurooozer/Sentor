# Atlas OS — Second-Brain IDE + Local Browser Plan (v2)

This document is the complete build plan for the next phase of Atlas OS.
...
Atlas OS is a **local-first second brain** for one user. It has three jobs:
...
- **AI chat** with three agents: **Vault** (default, research + memory), **Atlas-Maker**
...
- **Minimal agents** — exactly Vault, Atlas-Maker, Coder. No more.
...
| CLI with vault memory loop | [cli/atlas.py](cli/atlas.py) |
...
AI Layer:
  Agents: Vault (default) · Atlas-Maker · Coder
...
**Problem:** Atlas-Maker will generate HTML pages that include Mermaid diagrams.
...
In Atlas-Maker's generated HTML pages, the agent must use:
```html
<script src="/vendor/mermaid.min.js"></script>
```
*not* the CDN URL.

Update Atlas-Maker's system prompt (Phase B) to enforce this path.
...
- Read vault/projects/atlas-os-mimari/index.html once as a template reference.`,
  },
  {
    id: "builtin:coder",
    name: "Coder",
...
**Acceptance (Phase B):**
Open agent picker → exactly 3 agents: Vault, Atlas-Maker, Coder. Vault is selected by default.

---

### PHASE C — Auto re-index after vault_write

**Goal:** A page written by Atlas-Maker is searchable on the next turn without manual intervention.
...
**Acceptance (Phase C):**
Ask Atlas-Maker to write a page on "photosynthesis". Wait 5 seconds. Switch to Vault agent,
...
6. Ask Atlas-Maker anything → vault_write fires → a new browser tab opens with the vault page.
...
| Voice → Atlas-Maker (Whisper hook → vault write) | [useWhisperRecording.ts](ide/src/modules/ai/hooks/useWhisperRecording.ts) | S |
...
- `ide/src/modules/ai/lib/agents.ts` — replace BUILTIN_AGENTS with Vault, Atlas-Maker, Coder
...
- `cli/atlas.py` (already has vault memory loop — Phase 0–E don't affect it)
...
| Model ignores "vault_search first" rule | Medium | Rule is in the first line of the system prompt. Acceptable failure mode: model answers from model knowledge, then user can ask Atlas-Maker to save it. |
| Atlas-Maker generates HTML with CDN mermaid link instead of `/vendor/mermaid.min.js` | Medium | Make the rule bold in the system prompt. Add a post-write validator that checks the written HTML and warns if a CDN mermaid link is found. |
...
2. Agent picker shows exactly **Vault, Atlas-Maker, Coder** with Vault as default.
3. With Atlas-Maker selected, asking "explain how DNS resolution works" triggers: `vault_search` → `web_search` → `web_fetch` → user approval card (HTML preview) → `vault_write` → a new browser tab auto-opens with the rendered page.
