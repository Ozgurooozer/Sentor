...
"""
Sentor CLI
  sentor index              rebuild the search index
  sentor search "q"         search pages
  sentor list [category]    list all pages, or pages in a category
  sentor open category/slug open a page in the default browser
"""
...
def _load_pages() -> list[dict]:
    if not INDEX_FILE.exists():
        print('\n  No index found. Run:  sentor index\n')
        sys.exit(1)
    try:
...
def cmd_search(query: str) -> None:
    ...
    if not terms:
        print('\n  Usage: sentor search "your query"\n')
        return
...
def cmd_chat(model_name: str | None) -> None:
    ...
    try:
        ...
        print('\n  Sentor API is not running. Start it with:  sentor serve\n')
        sys.exit(1)

    print(f'\n  {_accent("Sentor")} chat  ·  model: {_dim(model)}')
...
def main() -> None:
    ap = argparse.ArgumentParser(
        prog='sentor',
        description='Sentor — local knowledge base',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            'examples:\n'
            '  sentor index\n'
            '  sentor search "semantic html"\n'
            '  sentor list\n'
            '  sentor list html\n'
            '  sentor open html/html-quality\n'
            '  sentor serve\n'
            '  sentor serve 8080\n'
            '  sentor chat\n'
            '  sentor chat --model qwen2.5-coder:7b\n'
            '\n'
            '  sentor new-task                       # interactive wizard\n'
            '  sentor task list\n'
            '  sentor task show pdf-reviewer\n'
            '  sentor run pdf-reviewer --wait\n'
            '  sentor provider status\n'
            '  sentor notify "build done"\n'
        ),
    )
