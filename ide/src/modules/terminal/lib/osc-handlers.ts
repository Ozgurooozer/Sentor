...
export function registerAtlasOpenHandler(
  term: Terminal,
  onAtlasOpen: (input: AtlasOpenInput) => void,
): () => void {
  const d = term.parser.registerOscHandler(8888, (data) => {
    const input = parseAtlasOpen(data);
    if (input) onAtlasOpen(input);
    return true;
  });
  return () => d.dispose();
}
...
function parseAtlasOpen(data: string): AtlasOpenInput | null {
