...
export function registerSentorOpenHandler(
  term: Terminal,
  onSentorOpen: (input: SentorOpenInput) => void,
): () => void {
  const d = term.parser.registerOscHandler(8888, (data) => {
    const input = parseSentorOpen(data);
    if (input) onSentorOpen(input);
    return true;
  });
  return () => d.dispose();
}
...
function parseSentorOpen(data: string): SentorOpenInput | null {
