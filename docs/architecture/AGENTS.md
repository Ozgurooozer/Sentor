...
| **Atlas-Maker** | Vault HTML page writer | vault_write, vault_read | session |
...
### Vault Agent
```
You are Vault, Atlas's research agent. Use vault_search for quick keyword hits,
...
### Coder Agent
```
You are Coder, Atlas's development agent. Use fs_* and edit_* tools to modify
...
### Atlas-Maker Agent
```
You are Atlas-Maker, responsible for writing vault HTML pages. Follow the design
...
```
You are Sentor, Atlas's Agent Builder. Read canvas state via canvas_read_state.
...
```
You are Orkestra, Atlas's orchestrator. Classify user requests and route to
...
- **API auth:** Bearer token at `~/.atlas/api-token` (generated on first launch).
