# Sentor — AI & Tool Calling Teknik Rapor
**Tarih:** 28 Mayıs 2026  
...
- `sentor:canvas-prompt` event'i emit ediyor → `CanvasAppShell` dinliyor → `orkestraStore.send()` çağırıyor
...
Sentor'ta birbirinden tamamen bağımsız iki AI motoru var:
...
createContextAwareTransport(deps)
      ↓
createSentorAgent({ keys, modelId, toolContext, ... })
      ↓
...
```
Sentor canvas AI. Node editor control. Reply in user's language. Be concise.
WS:C:\Sentor
NODES:
...
### Kısa vadeli
3. **Orkestra → AI SDK migration** — `orkestraStore.send()` direct fetch yerine `createSentorAgent` kullanacak şekilde refactor. JSON text scanning kaldırılır, structured tool calling gelir.
