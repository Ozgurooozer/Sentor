...
**Tarih:** 2026-05-19
**Status:** accepted
**Decision:** `ide/src/modules/ai/lib/agents.ts` içindeki BUILTIN_AGENTS sabitinden Sentor entry'si silindi. Built-in agent sayısı 4 → 3 (Vault, Sentor-Maker, Coder).
...
**Tarih:** 2026-05-19
**Status:** accepted
**Decision:** 6 prototip varyantı (htlas, otlas, qtlas, taslak, tlas, sentor-root) kök seviyede `prototypes/` altına `git mv` ile taşındı.
...
**Reason:** Sentence-transformers ~2 GB indir + 5 dk setup; Sentor "sıfır bağımlılık" hedefiyle çelişir.
**Alternatives:** Her ikisini de tut — reddedildi, iki backend bakım yükü; kullanıcı için Ollama tek komut.
**Impact:** `tools/embedder.py` ve `api/server.py:_embed_query` revize; CONFIG_FILE (`.sentor-embed.json`) sadece `ollamaUrl`/`ollamaModel` taşır.
